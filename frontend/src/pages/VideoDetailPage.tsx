...
export const VideoDetailPage = () => {
  ...
  return (
    <div>
      ...
      {isLoading && <p className="mt-6 text-sm text-gray-400">Loading video...</p>}

      {isError && (
        <div role="alert" className="mt-6 rounded bg-red-500/10 px-4 py-3 text-sm text-red-400">
      ...
      {video && (
        <>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <h1 className="text-2xl font-bold">{video.title ?? video.original_filename}</h1>
          ...
          {video.status === 'failed' && (
            <div role="alert" className="mt-6 rounded bg-red-500/10 px-4 py-3 text-sm text-red-400">
              ...
              {retryVideo.isError && (
                <p className="mt-2 text-xs text-red-400">{retryVideo.error.message}</p>
              )}
            </div>
          )}
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              ...
              {video.clips.map((clip) => (
                <Card key={clip.id} className="flex flex-col">
                  ...
                  />
                  <h2 className="mt-2 truncate text-sm font-medium">{clip.hook_title}</h2>
                  <span className="mt-1 inline-flex w-fit items-center rounded-full bg-nflix-red/20 px-2 py-0.5 text-[11px] font-medium text-red-400">
